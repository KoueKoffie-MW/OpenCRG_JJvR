...
if ~isempty(fixtures)
    ...
    [metricNames, metricValues, metricUnits, metricNotes] = addMetric( ...
        metricNames, metricValues, metricUnits, metricNotes, ...
        "crg_eval_uv2z.10k", timeit(@() crg_eval_uv2z(data, puv)), "seconds", fixtures(1).Name);
    [metricNames, metricValues, metricUnits, metricNotes] = addMetric( ...
        metricNames, metricValues, metricUnits, metricNotes, ...
        "crg_eval_uv2xy.10k", timeit(@() crg_eval_uv2xy(data, puv)), "seconds", fixtures(1).Name);
    pxy = crg_eval_uv2xy(data, puv(1:min(1000, size(puv, 1)), :));
    [metricNames, metricValues, metricUnits, metricNotes] = addMetric( ...
        metricNames, metricValues, metricUnits, metricNotes, ...
        "crg_eval_xy2uv.1k", timeit(@() crg_eval_xy2uv(data, pxy)), "seconds", fixtures(1).Name);
end
